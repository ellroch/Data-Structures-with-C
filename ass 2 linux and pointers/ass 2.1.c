...
//- - -- - -- - -- - -- - -- - -- - -- - -- - -- - - pointerAmp (int &pointer)

int pointerAmp (int &p, int x){
 int *pa;
 pa= &p; 
...
int switchGet (){
 ...
 printf("		(1) pass pointer plain 	\\(int pointer)\n");
 printf("		(2) pass *pointer 		\\(int *pointer)\n";
 printf("		(3) pass &pointer		\\(int &pointer)- just to cover all bases\n\n");

 printf("		(4) pass value plain 		\\(int value)\n");
 printf("		(5) pass *value		\\(int *value)\n");
 printf("		(6) pass &value		\\(int &value)\n");
 printf("	   : ");
...
while (switchNum!= 0){
 ...
 switch (switchNum){
  ...
  case 3: 
   while (xMain<2){
	returnPointer[xMain]=pointerAmp(pointer,xMain);
    }
   break;
  case 4:
   while (xMain<2){
  ...
  case 6:
   while (xMain<2){
	returnValue[xMain]=valueAmp(value,xMain);
    }
	break;
  case 0: return 0;
   break;
