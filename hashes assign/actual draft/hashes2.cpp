...
text editor whining at me about the comparison between long and unsigned while I
write the rest of this file.
*/
int int_exp(int number, long power){
  // if negative power call a logarithmic function like log(num, power*-1)
  long counter=0;
  long value=1; //if power is 0 then value is 1... if power is 1 answer is number...etc
  while (counter<power){
    value= value*number;
    counter++;
  }
...
*/
int extract_digit(int target_digit, long source_number){
  printf("\ntest e_d 1");
  long extraction;
  printf("\ntest e_d 2");
  // because power -1 is not actually accounted for in int_exp we cant allow target digit 0 in this function
  extraction= ( source_number/((long)pow(10,target_digit-1)) )%10;
  printf("\ntest e_d 4");
  return extraction;
  printf("\ntest e_d 5");
}
//------------------------------------------------------------this is for hash_c
int extract_digit_2(int target_digit, long source_number, long length){
  printf("\ntest e_d 1");
  long extraction=0;
  printf("\ntest e_d 2");
  while(extraction==0){
  // because power -1 is not actually accounted for in int_exp we cant allow target digit 0 in this function
  extraction= ( source_number/((long)pow(10,length-target_digit-1) )%10);
  printf("\ntest e_d 4");
   target_digit--;
  }
  printf("\ntest e_d 5");
...
*/
int midsquare(int starting_value, long bot_range, long top_range){ //top_range and bot_range signify the ends of the target block
  long mid=0;
  long length=(top_range-bot_range)+1;
  // 5-3 would be 2... but we would want 3 numbers in the places 3, 4, and 5... so +1 to the subtraction
  long *extract_hold= new  long long[length]; //new int[length];
//  extract_hold= calloc(length, sizeof(int)); //acts like long array[length] ... but c++ doesnt like that syntax so :/
  long counter=0;
...
*/
int rotation(int number, long rotation_count, long number_of_digits){// rotates numbers like so: 10000-> 01000 -> 00100 ... ...
  long counter=0;
  long temp;
  while (counter<rotation_count){
    temp=extract_digit(1, number);
    number=number/10;
    temp=temp*(long)pow(10,number_of_digits-1);
    number=number+temp;
    counter++;
  }
  return number;
...
*/
int flip(int num){
  long length=num_len(num);// long len() determines the number of digits in length the number is
  long *extract_hold= new long long[length];
  long counter=0;
  while (counter<length){
...
int fold(int num){
  ...
  printf("\ntest b 1");
  long num_len=num_len(num);
  printf("\ntest b 2");
  long chunk_len=num_len/2;
  printf("\ntest b 3");
  long flipping_chunk=midsquare(num, 1, chunk_len);
  printf("\ntest b 4");
                //flip it... (becomes 54)
  ...
  num/=(long)pow(10,chunk_len/2);
  printf("\ntest b 6");
              //and tack the flip onto the back (54 + 123 -> 54123)
  flipping_chunk*=(long)pow(10,chunk_len-(chunk_len/2));
  printf("\ntest b 7");
  num+=flipping_chunk;
  printf("\ntest b 8");
...
int hash_a(int hash){
  ...
  hash=hash*((long)pow(master_extract,3));
  cout<< "\nmaster xtract: " <<master_extract<<"\t hash: "<<hash<< "\thash len: "<<hash_len;

  printf("\ntest c 5");
  hash_len=num_len(hash);
  cout<< "\nmaster xtract: " <<master_extract<<"\t hash: "<<hash<< "\thash len: "<<hash_len;
  ...
  long temp=extract_digit_2(master_extract%hash_len, hash, hash_len);
  printf("\ntest c 7");
  cout<< "\nmaster xtract: " <<master_extract<<"\t hash: "<<hash<< "\thash len: "<<hash_len;

  long counter=5;
  while(hash_len<6){
    hash=(long)pow(hash,counter);
    hash_len=num_len(hash);
    counter--;
    hash-=master_extract;
  }
  ...
  long temp2=master_extract%hash_len+1;
  cout << hash<<":-: "<< temp2;
  cout<< "\nmaster xtract: " <<master_extract<<"\t hash: "<<hash<< "\thash len: "<<hash_len;
...
//-------------------------------------------------probe list
Node* probe_list(List &list, long address){
...
//-------------------------------------------------probe bucket
Bucket_Node* probe_bucket_list(Bucket_List* list, long address){
...
...
they are long functions, returning the number of collisions
...
...
/*-------------------------------------------------insert list a
*/
int insert_list_a(List list, long key){
  long collisions=0;
  long hash=hash_a(key);
  long address=hash%SIZE;
  printf("\n testing 2");
...
/*-------------------------------------------------insert list b
*/
int insert_list_b(List list, long key){
  long collisions=0;
  printf("\ntesting a");
  long hash=hash_b(key); //hash b is a mod of the list size so we dont need to... mod it here
  printf("\ntesting b");
  long address=hash%SIZE;
  printf("\ntesting c");
  Node* node=probe_list(list, address);
...
*/
int insert_bucket_list_a(Bucket_List* bucket_list, long key){
  long collisions=0;
  long hash=hash_a(key);
  long address=hash%SIZE;
...
*/
int insert_bucket_bucket_list_b(Bucket_List* bucket_list, long key){
  long collisions=0;
  long hash=hash_b(key);
  long address=hash%SIZE;
  Bucket_Node* node=probe_bucket_list(bucket_list, address);
  printf("\n testing 27");
  if (node->nodeIsFull){//its a bool
...
int main(){ // on hash 'a' I had fun :D ... i wanted to see if crazy crazy complicated hashes actually pay off or nah... so we got some juxtaposition going on here... I have a hunch that this much is excessive and uselessly redundant... but hey :D I get extra credit for having used each of the funky hash algorithms in the hash right?
  ...
  long la_failed_inserts=0;
  long la_collisions=0;

  long lb_failed_inserts=0;
  long lb_collisions=0;

  long ba_failed_inserts=0;
  long ba_collisions=0;

  long bb_failed_inserts=0;
  long bb_collisions=0;

  long counter_l=0; //this is the counter for the loop operating the switch statement for iterating over each list with the same key
  long counter=0;  // this is the counter for the outer loop, generating the 35 keys
